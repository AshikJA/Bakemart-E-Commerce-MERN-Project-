...
function Cart() {
  ...
  const handleUpdateQuantity = (item, delta) => {
    ...
    if (newQty < 1) return;
    const currentStock = item.selectedVariant ? item.selectedVariant.stock : item.stock;
    if (delta > 0 && currentStock <= item.quantity) {
      toast.warning('Not enough stock available');
      return;
    }
    addToCart(item, delta, item.selectedVariant);
  };
      ...
      {isConfirmingClear && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl">
            <h3 className="text-2xl font-black text-[#6B3F1F] mb-4">Clear Cart?</h3>
            <p className="text-gray-600 mb-8 font-medium">Are you sure you want to remove all items from your cart? This action cannot be undone.</p>
