...
import { FiCheckCircle, FiPlus, FiArrowLeft } from 'react-icons/fi';
      ...
      <div className="max-w-6xl mx-auto grid lg:grid-cols-3 gap-8 lg:gap-10">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white p-6 sm:p-8 rounded-[32px] shadow-sm border border-[#D4A96A]/20">
            ...
            <div className="space-y-4">
              <label className={`flex items-center gap-4 p-4 rounded-2xl border-2 cursor-pointer transition-colors ${paymentMethod === 'COD' ? 'border-[#6B3F1F] bg-[#6B3F1F]/5' : 'border-gray-100 hover:border-[#D4A96A]/50'}`}>
                <input type="radio" name="payment" value="COD" checked={paymentMethod === 'COD'} onChange={() => setPaymentMethod('COD')} className="w-5 h-5 text-[#6B3F1F]" />
                ...
                <div key={item._id} className="flex justify-between items-center text-sm gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-lg overflow-hidden bg-white/10 flex-shrink-0">
                       <img 
                         src={item.image ? (item.image.startsWith('http') ? item.image : `${api.defaults.baseURL.replace('/api', '')}/uploads/${item.image}`) : 'https://via.placeholder.com/50'} 
